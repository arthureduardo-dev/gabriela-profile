...
function App() {
  ...
  const handleDotClick = (index: number) => {
    ...
    const targetClip = clipRefs.current[index];

    if (carousel && targetClip) {
      const carouselWidth = carousel.offsetWidth;
      const clipWidth = targetClip.offsetWidth;
      const clipOffsetLeft = targetClip.offsetLeft;
      
      const scrollTo = clipOffsetLeft - (carouselWidth / 2) + (clipWidth / 2);
            ...
            >
              {clipsData.map((clip, index) => (
                <div
                  ...
                  data-index={index}
                  className="group relative flex-shrink-0 w-full md:w-3/5 snap-center rounded-2xl overflow-hidden border border-dark-purple/30 transition-all duration-300 hover:border-dark-purple/50"
                >
                  <video
